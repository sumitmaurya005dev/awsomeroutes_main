...
type FormState = {
  ...
  name: string;
  slug: string;
  location_type: "place" | "activity_spot" | "attraction" | "other";
...
export function LocationForm({
  ...
  const [form, setForm] = React.useState<FormState>({
    ...
    slug: initial?.slug ?? "",
    location_type:
      initial?.location_type === "activity_spot" ||
      initial?.location_type === "attraction" ||
      initial?.location_type === "other"
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = initial
      ? await updateLocationAction(initial.id, values)
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <label className="space-y-2 text-sm font-medium">
            ...
            >
              ...
              <option value="activity_spot">Activity spot</option>
        ...
        <div className="flex justify-end gap-3 border-t pt-5">
          <Button
            ...
            onClick={() => {
              ...
              else router.push("/home/locations");
            }}
          >
            Cancel
          </Button>
          <Button type="submit" disabled={saving}>
            {saving ? "Saving..." : initial ? "Save changes" : "Save location"}
          </Button>
        </div>
      </form>
      <MediaPickerDialog
        ...
        folder={MEDIA_FOLDERS.LOCATIONS}
        fileNamePrefix={form.slug || "location"}
        altText={form.name || "Location image"}
        onSelect={(asset) =>
          setForm((current) => ({
            ...current,
